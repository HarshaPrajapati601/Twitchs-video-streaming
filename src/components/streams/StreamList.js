...
class StreamList extends React.Component {
        ...
        if(currentStream?.userId === this.props.currentUserID) {
            return (
                <div className="right floated content">
                    <Link to={`/streams/edit/${currentStream.id}`} className="ui button primary">
                        Edit
                    </Link>
                    <Link  to={`/streams/delete/${currentStream.id}`} className="ui button negative">
                        Delete
                    </Link>
                </div>
            )
    ...
    renderList() {
        return this.props.streams.map(stream => {
            return (
                <div className="item" key={stream.id} onClick>
